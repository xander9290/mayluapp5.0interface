...
export const formatoFecha = (d) => {
  ...
  const fullDate = `${dia < 10 ? "0" + dia : dia}/${
    mes < 10 ? "0" + mes : mes
  }/${ano} ${hora > 12 ? hora - 12 : hora}:${min < 10 ? "0" + min : min} ${
    hora > 12 ? "p.m." : "a.m."
  }`;

  const justHour = `${hora > 12 ? hora - 12 : hora}:${
    min < 10 ? "0" + min : min
  } ${hora > 12 ? "p.m." : "a.m."}`;

  return [fullDate, justHour];
};
