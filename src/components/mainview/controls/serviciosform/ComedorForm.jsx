...
function ComedorForm({ show, onHide, setCapturaForm }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (cuenta._id) {
      ...
      updateCuenta(cuenta._id, newComedor);
      onHide();
    } else {
      const newComedor = {
        ...cuenta,
      ...
      if (verifyExisteMesa(cuentas, comedor.torreta)) {
        alert("mesa no disponible".toUpperCase());
        mesaInputRef.current.focus();
        return;
      }
      createCuenta(newComedor, (res) => {
        if (res) {
          onHide();
          setTimeout(() => {
            setCapturaForm();
